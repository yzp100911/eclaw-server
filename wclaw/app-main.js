    var currentBackend = localStorage.getItem('wclaw_backend') || 'openclaw';
    ...
    function updateHeaderBackend() {
        const labelMap = { hermes: 'Hermes', xcrab: 'xCrab', openclaw: 'OpenClaw' };
    ...
    function toggleBackend() {
        ...
        updateHeaderBackend();
        const labelMap = { hermes: 'Hermes Agent', xcrab: 'xCrab', openclaw: 'OpenClaw' };
    ...
    async function loadHistory() {
        ...
        localHistory.forEach(msg => {
            ...
            if (msg.role === 'user') {
                ...
                const historyBackend = msg.backend || 'openclaw';
                const historyBadge = historyBackend === 'hermes' ? 'Hermes' : historyBackend === 'xcrab' ? 'xCrab' : historyBackend === 'cron' ? '定时' : 'OpenClaw';
                box.insertAdjacentHTML('beforeend', `
            ...
            try {
                ...
                // ====== xCrab 工具调用实时状态 ======
                    ...
                    if (_rowEl) {
                        ...
                        // 移除旧工具状态条
                        var _oldStatus = _rowEl.querySelector('.xcrab-tool-status');
                        if (_oldStatus) _oldStatus.remove();
                        // 创建新工具状态条
                        var _toolEl = document.createElement('div');
                        _toolEl.className = 'xcrab-tool-status xcrab-tool-running';
                        _toolEl.style.cssText = 'margin:6px 12px;padding:6px 10px;background:#fff3e0;border-radius:6px;font-size:13px;color:#e65100;display:flex;align-items:center;gap:6px;border:1px solid #ffe0b2;';
                        _toolEl.id = 'tool-status-' + _replyId;
                        var _stepInfo = _tcTotal > 1 ? ' [' + _tcIdx + '/' + _tcTotal + ']' : '';
                        _toolEl.innerHTML = '<span style="flex-shrink:0;">🔧</span> 正在执行: <code style="background:#ffe0b2;padding:1px 6px;border-radius:4px;font-size:12px;">' + escapeHtml(_tcName) + '</code>' + _stepInfo + '<span class="tool-elapsed" style="margin-left:auto;color:#bf360c;font-size:12px;font-weight:500;">⏱ 0s</span>';
                        _rowEl.appendChild(_toolEl);
                    }
                } else if (data.type === 'tool_progress') {
                    resetXcrabIdle();
                    var _elapsed = data.data && data.data.elapsed;
                    var _progressRowEl = document.getElementById('row-reply-' + msgId);
                    if (_progressRowEl) {
                        var _elapsedEl = _progressRowEl.querySelector('.tool-elapsed');
                    ...
                    var _resultRowEl = document.getElementById('row-reply-' + msgId);
                    if (_resultRowEl) {
                        var _resultStatus = _resultRowEl.querySelector('.xcrab-tool-status');
                        if (_resultStatus) {
                            _resultStatus.className = 'xcrab-tool-status xcrab-tool-done';
                            _resultStatus.style.cssText = 'margin:6px 12px;padding:6px 10px;background:#e8f5e9;border-radius:6px;font-size:13px;color:#2e7d32;display:flex;align-items:center;gap:6px;border:1px solid #c8e6c9;';
                            _resultStatus.innerHTML = '<span style="flex-shrink:0;">✅</span> 工具完成: <code style="background:#c8e6c9;padding:1px 6px;border-radius:4px;font-size:12px;">' + escapeHtml(_resultName) + '</code><span style="margin-left:auto;color:#558b2f;font-size:12px;">(' + _fmtTime + ')</span>';
                        }
                    }
